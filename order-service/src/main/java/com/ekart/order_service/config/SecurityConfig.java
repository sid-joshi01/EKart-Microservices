...
public class SecurityConfig {
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http.csrf(csrf -> csrf.disable())
                ...
                .authorizeHttpRequests(auth ->
                        auth.requestMatchers(HttpMethod.GET,"/api/order/total-amount").permitAll() // Currently I don't have frontend so unable to add token with this request that's why I'm making it public for now
    ...
    public CorsConfigurationSource corsConfigurationSource() {
        ...
        config.setAllowedOrigins(List.of("http://localhost:8080", "http://localhost:8086")); // 👈 frontend origin
